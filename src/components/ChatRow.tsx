...
function AgentChatRow({ message, agent }: { message: ChatMessage; agent: AgentChatMeta }) {
  ...
  return (
    <div className="flex min-w-0 max-w-full justify-start">
      <div
        className={cn(
          "w-full min-w-0 rounded border bg-white px-3 py-2 text-[12px] leading-5",
          isError ? "border-[var(--danger)] bg-[var(--panel-2)] text-[var(--danger)]" : "border-[var(--line)] text-[var(--charcoal)]",
        )}
      >
          ...
          <ul className="mt-1.5 grid gap-1 text-[11px] leading-4 text-[var(--muted)]">
            {agent.details.map((detail, index) => (
              <li key={`${agent.runId}-${agent.phase}-${index}`} className="flex min-w-0 gap-1.5">
                <span className="mt-[7px] h-1 w-1 shrink-0 bg-[var(--primary)]" />
...
export function ChatRow({ message }: { message: ChatMessage }) {
  ...
  const levelClass =
    message.level === "error"
      ? "border-[var(--danger)] bg-[var(--panel-2)] text-[var(--danger)]"
      : message.level === "success"
        ? "border-[var(--line-strong)] bg-[var(--panel-2)] text-[var(--primary-strong)]"
        : isUser
  ...
  return (
    <div className="flex min-w-0 max-w-full justify-start">
      <div className={cn("w-full min-w-0 rounded border px-3 py-2 text-[12px] leading-5", levelClass)}>
